...
router
      ...
      .get('/', (req, res, next) => {
        ...
        //  next(); No es necesario por que la pila de middlware termina
      ...
      .post('/', (req, res, next) => {
        req.getConnection( (err, movies) => {
            ...
            movies.query('INSERT INTO movie SET ?', movie, (err, rows) => {
                return (err) ? next(new Errror('No se pudo ingresar el registro')) : res.redirect('/');
            })
        })
      })
      .get('/editar/:movie_id', (req, res, next) => {
          ...
          req.getConnection( (err, movies) => {
            movies.query('SELECT * FROM movie WHERE movie_id = ?', movie_id , (err, rows) => {
                ...
                if(err){
                  next(new Error('Registro no econtrado') );
                }else{
                  let locals = {
